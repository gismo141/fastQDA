...
void DocumentWidget::showPage(int page) {
  ...
  QImage image = doc->page(currentPage)
                 ->renderToImage(scaleFactor * physicalDpiX()
...
void DocumentWidget::annotate() {
  Poppler::TextAnnotation* annotation = new Poppler::TextAnnotation(
    Poppler::TextAnnotation::InPlace);
  // normalized coordinates: (0,0) is top-left, (1,1) is bottom-right
  annotation->setBoundary(QRectF(-0.1, -0.1, 0.2,
                                 0.2));
  annotation->setContents("Hello, world!");
  doc->page(currentPage)->addAnnotation(annotation);
  QString path = this->path;
  QString temp = this->path + ".tmp";

  Poppler::PDFConverter* converter = doc->pdfConverter();
  converter->setOutputFileName(temp);
  converter->setPDFOptions(converter->pdfOptions() |
                           Poppler::PDFConverter::WithChanges);
  converter->convert();
  setDocument(temp);
  converter = doc->pdfConverter();
  converter->setOutputFileName(path);
  converter->setPDFOptions(converter->pdfOptions() |
                           Poppler::PDFConverter::WithChanges);
  converter->convert();
  delete converter;
