...
codeList::codeList(QMainWindow* parent) {
  ...
  QObject::connect(setCode, SIGNAL(clicked()), thisParent
                   , SLOT(setCodeinSelection()));
  QObject::connect(codeTree, SIGNAL(itemDoubleClicked(QTreeWidgetItem*, int))
                   , codeSetup, SLOT(changeParameters(QTreeWidgetItem*, int)));
}

void codeList::setCodeinSelection(void) {
  reinterpret_cast<fastQDA*>(thisParent)->setCodeinSelection(
    codeTree->currentItem());
}

void codeList::addCode(const QString& name, const QString& definition
  ...
  if (newCode) {
    ...
    reply = QMessageBox::question(this, "WARNING!", "Overwrite existing code?"
...
QTreeWidgetItem* codeList::addCodeToList(code* code) {
  ...
  if (!theCodeItem) {
    ...
    } else {  // The code has no headcode so it is a headcode itself
